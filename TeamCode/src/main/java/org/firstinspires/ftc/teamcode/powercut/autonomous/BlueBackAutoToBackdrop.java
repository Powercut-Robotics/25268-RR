...
public class BlueBackAutoToBackdrop extends OpMode {
    ...
    public void init() {
        ...
        toRightSpike = drive.actionBuilder(drive.pose)
                .splineTo(new Vector2d(5, 38), Math.toRadians(225))
                .build();

        toLeftBackdrop = drive.actionBuilder(new Pose2d(17, 38, Math.toRadians(-45)))
                .turn(Math.toRadians(-135))
                .strafeTo(new Vector2d(51, 42))
                .build();

        toCentreBackdrop = drive.actionBuilder(new Pose2d(12, -32, Math.toRadians(270)))
                .turn(Math.toRadians(-90))
                .strafeTo(new Vector2d(51, 35))
                .build();

        toRightBackdrop = drive.actionBuilder(new Pose2d(17, -38, Math.toRadians(225)))
        ...
        switch (recordedPropPosition) {
            case LEFT:
                Actions.runBlocking(new SequentialAction(
                        ...
                        arm.gripTuck(),
                        new SleepAction(0.5),
                        new ParallelAction(arm.armToResetPosition(), arm.wristToResetPosition(), arm.gripRelease(), parkFromLeft)
                ));
                break;
            case MIDDLE:
                Actions.runBlocking(new SequentialAction(
                        ...
                        arm.gripTuck(),
                        new SleepAction(0.5),
                        new ParallelAction(arm.armToResetPosition(), arm.wristToResetPosition(), arm.gripRelease(), parkFromCentre)
                ));
                break;
            case RIGHT:
                Actions.runBlocking(new SequentialAction(
                        ...
                        arm.gripTuck(),
                        new SleepAction(0.5),
                        new ParallelAction(arm.armToResetPosition(), arm.wristToResetPosition(), arm.gripRelease(), parkFromRight)
                ));
                break;
        }
